...
Future<String> saveBackupFile(
  ...
  final String filename = "easyWallet_backup_${dateTime}_$randomValue.$fileExt";

  log("[EW Sync] Writing to ${path.join(saveDir.path, filename)}");

  final File f = File(path.join(saveDir.path, subfolder ?? "", filename));
  f.createSync(recursive: true);
  f.writeAsStringSync(backupContent);

  log("[EW Sync] Write successful. See file at: ${f.path}");
...
Future<ExportStatus> showFileSaveDialog(
  ...
  log("[EW Sync] shareSuccess $shareSuccess $uri");
